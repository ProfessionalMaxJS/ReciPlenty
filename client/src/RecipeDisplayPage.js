    ...
    fetch("/backend/logged_in")
      ...
      .then((d) => {
        ...
        if (d.logged_in === false) {
          toTheHouse("/");
          alert(
            "Sorry, this Page is Only for Saved Recipes (a Feature only Available to Members Whov've Signed Up or Logged In)"
          );
        }
      });
  }, []);
      ...
      >
        {id < 999 && (
          ...
          >
            ...
            <p style={{ fontFamily: "Alice, serif" }}>
              {" "}
              {checked
                ? "I HAVE Made this Recipe Already!"
        ...
        <CardMedia
          component="img"
          height="350"
          image={id < 999 ? foodPicUrl : recipe.api_img}
          alt={id < 999 ? recipe.title : recipe.title}
        />
        <Typography
          style={{
        ...
        {id < 999 ? (
          <Link
            to="EditPage"
            style={{ color: "white", background: "#1976D2", paddingTop: "5px" }}
          >
            EDIT!
          </Link>
        ) : (
          <Button
            variant="contained"
            style={{ fontFamily: "Alice, serif", marginTop: "2px" }}
