...
useEffect(() => {
  ...
  const [newBFPasswordConfirmError, setNewBFPasswordConfirmError] =
    useState(false);
  const [newBFPasswordConfirmHelper, setNewBFPasswordConfirmHelper] =
    useState("");
  ...
  const handleSignUp = () => {
    ...
    if (!newBonaFides.name) {
      ...
      setNewBFPasswordError(true);
      setNewBFPasswordConfirmError(true);
      return setNewBFPasswordConfirmHelper(
...
`https://api.spoonacular.com/recipes/random?number=1&tags=lunch&apiKey=${spoonacularApiKey}`
  ...
  const handlePairing = () => {
    fetch("https://api.punkapi.com/v2/beers/random")
      ...
      .then((d) => {
        //console.log(d)
        let newStr = `Try the beer "${d[0].tagline
          .toString()
          .slice(0, -1)}" alongside `;
  ...
  return (
    <>
      {!loggedIn && (
        <div>
          ...
          >
            ...
            >
              <TextField
                helperText={newBFNameHelper}
                error={newBFNameError}
                style={{ margin: "3px 0px 3px 0px", width: "100%" }}
                onChange={handleNewBonaFides}
                required
                value={newBonaFides.name}
                name="name"
                label="Username"
              />
              <TextField
                helperText={newBFPasswordHelper}
                error={newBFPasswordError}
                style={{ margin: "3px 0px 3px 0px", width: "100%" }}
                onChange={handleNewBonaFides}
                required
                type="password"
                value={newBonaFides.password}
                name="password"
                label="Password"
              />
              <TextField
                helperText={newBFPasswordConfirmHelper}
                error={newBFPasswordConfirmError}
                style={{ margin: "3px 0px 3px 0px", width: "100%" }}
                onChange={handleNewBonaFides}
                required
                type="password"
                value={newBonaFides.password_confirmation}
            ...
            >
              <TextField
                helperText={returnBFNameHelper}
                error={returnBFNameError}
                style={{ margin: "3px 0px 3px 0px", width: "100%" }}
                onChange={handleReturnBonaFides}
                required
                label="Username"
                name="name"
                value={returnBonaFides.name}
              />
              <TextField
                helperText={returnBFPasswordHelper}
                error={returnBFPasswordError}
                style={{ margin: "3px 0px 3px 0px", width: "100%" }}
                onChange={handleReturnBonaFides}
                required
                label="Password"
                name="password"
                type="password"
                value={returnBonaFides.password}
              />
              <Button
                style={{
                  margin: "3px 0px 3px 0px",
      ...
      <div style={{ textAlign: "center", fontFamily: "Alice, serif" }}>
        <h1>
          Welcome to ReciPlenty, a place for all the best food ideas to
          mingle...
        </h1>
        <p>
          On this app, you can note down your best new ideas, or look up recipes
          for foods you've always wanted to try. But while you're, here, you can
          also:
        </p>
      </div>
      <div
      ...
      >
        ...
        >
          ...
          <Button
            style={{ fontFamily: "Alice, serif", margin: "5px 2px 5px 2px" }}
            variant="contained"
            onClick={handleTrivia}
          >
            Click to Learn a Random Food Fact
          </Button>
        </Card>
        <Card
          sx={{
        ...
        >
          ...
          <a href={source} style={{ fontFamily: "Alice, serif" }}>
            {source}
          </a>
          <Button
            style={{ fontFamily: "Alice, serif", margin: "5px 2px 5px 2px" }}
            ...
            onClick={handleBlog}
          >
            Click for a Link to a Random Food Blog
          </Button>
        </Card>
        <Card
          sx={{
        ...
        >
          ...
          >
            Click for a Random Beer-Food Pairing
          </Button>
        </Card>
      </div>
      ...
      <p>
        ...
        <a href="https://punkapi.com/" style={{ fontFamily: "Alice, serif" }}>
          {" "}
          Punk API
        </a>
        <br />
        Blogs, Trivia and Searchable Recipes powered by{" "}
        <a
